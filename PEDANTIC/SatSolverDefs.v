...
Definition coreStructures v0 v1 v2 v3 v4 : absStateBasic :=
        TREE(!!clauses,v0,#sizeof_clause,(#next_offset::nil)) **
        TREE(!!assignments_to_do_head,v1,#sizeof_assignment_stack,(#next_offset::nil)) **
        TREE(!!stack,v2,#sizeof_assignment_stack,(#next_offset::nil)) **
        ARRAY(!!assignments,#var_count,v3) **
        ARRAY(!!watches,#var_count,v4).

Definition treeInArray (tr : nat) (ar : nat) (l : nat) : absStateBasic :=
        (AbsAll TreeRecords(v(tr))
            ([--(v(S(tr)),v(0))-->stack_var_offset <<<< #var_count] **
             ([--(v(S(tr)),v(0))-->stack_val_offset ==== #1] *\/* [--(v(S(tr)),v(0))-->stack_val_offset ==== #2]) **
             ([nth(v(S(ar)),--(v(S(tr)),v(0))-->stack_var_offset)====--(v(S(tr)),v(0))-->stack_val_offset]) **
             (AbsAll TreeRecords(nth(find(v(S(tr)),v(0)),#2))
                 ([~~(--(v(S(S(tr))),v(1))-->stack_var_offset====--(nth(find(v(S(S(tr))),v(1)),#2),v(0))-->stack_var_offset)])))).

Definition arrayInTree tr ar : absStateBasic :=
        (AbsAll range(#0,#(var_count))
            ([nth(v(S(ar)),v(0))====#0] *\/*
             AbsExists (TreeRecords(v(S(tr))))
                  ([(--(v(S(S(tr))),v(0))-->stack_var_offset====v(1) //\\
                   --(v(S(S(tr))),v(0))-->stack_val_offset====nth(v(S(S(ar))),v(1)))]) )).

Definition treeEquivArray tr ar l : absStateBasic :=
    treeInArray tr ar l **
    arrayInTree tr ar.

Definition validBackPointers tr prev_offset next_offset : absStateBasic :=
        (AbsAll TreeRecords(v(tr))
            ([
              (--(v(S(tr)),v(0))--->(prev_offset)====#0 //\\ (nth(v(S(tr)),v(0))====v(0)) \\//
               (--(v(S(tr)),v(0))--->(prev_offset) inTree v(0) //\\
                --(v(S(tr)),--(v(S(tr)),v(0))--->(prev_offset))--->(next_offset)====v(0)))
             ])).

Definition assignmentConsistent cl c a : absStateBasic :=
    (AbsExists range(#0,#(var_count))
        ([
(--(N(cl),N(c))--->(#positive_lit_offset++++v(0)) //\\
           (nth(N(a),v(0))====#2 \\// nth(N(a),v(0))====#0)) \\//
          (--(N(cl),N(c))--->(#negative_lit_offset++++v(6)) //\\
           (nth(N(a),v(0))====#1 \\// nth(N(a),v(0))====#0))
])).

Definition watchVariablesExists tr c : absStateBasic :=
            (AbsAll range(#0,#(var_count))
                ([
                 (--(tr,c)--->(#watch_var_offset++++v(0))====#0) \\//
                  (--(tr,c)--->(#positive_lit_offset++++v(0))) \\//
                  (--(tr,c)--->(#negative_lit_offset++++v(0)))])).

Definition watchVariablesLinkedIffSet tr c a : absStateBasic :=
    (AbsAll range(#0,#(var_count))
        ([
             (~~(--(N(tr),N(c))--->(#watch_var_offset++++v(0))====#0) //\\
             (~~(--(N(tr),N(c))--->(#watch_prev_offset++++v(0))====#0) \\//
               nth(v(4),v(6))====v(5))) \\//
             (--(N(tr),N(c))--->(#watch_var_offset++++v(0))====#0 //\\
              --(N(tr),N(c))--->(#watch_prev_offset++++v(0))====#0 //\\
              ~~(nth(N(a),v(0))====(N(c))))])).

Definition twoWatchVariables tr c : absStateBasic :=
    (SUM(range(#0,#(var_count)),ite((--(tr,c)--->(#watch_var_offset++++v(0))),(#1),(#0)),#2)).

Definition onlyOneUnassigned tr c a : absStateBasic :=
    SUM(range(#0,#(var_count)),
     (((--(N(tr),N(c))--->(#positive_lit_offset++++v(0))) \\// (--(N(tr),N(c))--->(#negative_lit_offset++++v(0)))) //\\
       ite(nth(a,v(0))====#0,#1,#0)),
     #1).

Definition unassignedVariablesAreWatches tr c a : absStateBasic :=
    (AbsAll range(#0,#(var_count))
       ([(#0<<<<--(N(tr),N(c))--->(#watch_var_offset++++v(0)) //\\
         (nth(a,v(0))====#0)) \\//
         (
             ((#0<<<<nth(a,v(0))) \\// ((--(N(tr),N(c))--->(#positive_lit_offset++++v(0))====#0 //\\
             --(N(tr),N(c))--->(#negative_lit_offset++++v(0))====#0))))])).

Definition mostRecentAssignedIsWatch tr c a st : absStateBasic :=
    (AbsAll range(#0,#(var_count))
        (AbsAll range(#0,#(var_count))
            (([--(N(N(tr)),N(N(c)))--->(#watch_var_offset++++v(0)) \\//
               ((((--(N(N(tr)),N(N(c)))--->(#positive_lit_offset++++v(0)))====#0 //\\ (--(N(N(tr)),N(N(c)))--->(#negative_lit_offset++++v(0))====#0)))) \\//
                        ~~(--(N(N(tr)),N(N(c)))--->(#watch_var_offset++++v(1))) \\//
                        nth(N(N(a)),v(1))====#0 \\// nth(N(N(a)),v(1))====#0 \\// v(0)====v(1)]) *\/*
                      (AbsExists TreeRecords(N(N(st)))
                          (([--(N(N(N(st))),v(0))-->stack_var_offset====v(2)]) **
                           (AbsExists TreeRecords(find(N(N(N(st))),v(0)))
                              ([--(N(N(N(N(st)))),v(0))-->stack_var_offset====v(1)]))))))).

Definition allButOneAssigned tr c a st : absStateBasic :=
    onlyOneUnassigned tr c a **
    (* The one unassigned literal is a watch--needs fixing? *)
    unassignedVariablesAreWatches tr c a **
    mostRecentAssignedIsWatch tr c a st.

Definition satisfyingAssignmentMade tr c a : absStateBasic :=
    (AbsExists range(#0,#(var_count))
        ([(--(N(tr),N(c))--->(#positive_lit_offset++++v(0)) //\\ nth(N(a),v(0))====#2) \\//
        (--(N(tr),N(c))--->(#negative_lit_offset++++v(0)) //\\ nth(N(tr),N(c))====#1)])).

Definition watchAfterSatisfyingAssignment tr c a st : absStateBasic :=
    (AbsAll range(#0,#(var_count))
        ([#0====nth(N(a),v(0))] *\/*
         ([--(N(tr),N(c))--->(#watch_var_offset++++v(0))====#0] **
          [#0<<<<nth(N(a),v(0))]) *\/*
         (AbsExists TreeRecords(N(st))
             ([--(N(N(st)),v(0))-->stack_var_offset====v(1)]) **
             (AbsExists TreeRecords(find(N(N(st)),v(0)))
                 ([
((#0 <<<< (--(N(N(tr)),N(N(c)))--->(#positive_lit_offset++++ --(N(N(st)),v(0))-->stack_var_offset))) //\\ (--(N(N(st)),v(0))-->stack_val_offset====#2))
                      
\\//
((#0 <<<< (--(N(N(tr)),N(N(c)))--->(#negative_lit_offset++++ --(N(N(st)),v(0))-->stack_var_offset))) //\\
                      (--(N(N(st)),v(0))-->stack_val_offset====#1))

]))
))).

Definition watchesUnassigned tr c a : absStateBasic :=
    (AbsAll range(#0,#(var_count))
        ([--(N(tr),N(c))--->(#watch_var_offset++++v(0))====#0 \\// nth(N(a),v(0))====#0])).

Definition invariant: absStateBasic :=
    (AbsExistsT (AbsExistsT (AbsExistsT (AbsExistsT (AbsExistsT (
        coreStructures v(0) v(1) v(2) v(3) v(4) **
        (* Assertions that the stack and assignments array contain the same set
           of assignments *)
        treeEquivArray 2 3 var_count **
        (* Assertion defining the prev pointer in the assignments_to_do
           doubly linked list *)
        validBackPointers 1 (#prev_offset) (#next_offset) **
        (AbsEach range(#0,#(var_count))
            ...
            (AbsExistsT
                ((Path((nth(v(4),v(5))), v(0), v(6), #sizeof_clause, ((#watch_next_offset++++v(5))::nil))) **
                 ...
                 validBackPointers 6 (#watch_prev_offset++++v(5)) (#watch_next_offset++++v(5)) **
        (AbsAll TreeRecords(v(0))
            (* The current assignment is consistent with the clause *)
           (assignmentConsistent v(0) v(5) v(3)) **
            (*
             ...
             * is true.
             *)
            watchVariablesExists v(0) v(5) **
            watchVariablesLinkedIffSet v(0) v(5) v(3) **
            (* Make sure there are precisely two watch variables per clause or all variables are watches,
               needs fixing? *)
            twoWatchVariables v(0) v(5) **
            ...
            (allButOneAssigned v(0) v(5) v(3) v(2) *\/*
            ...
            ( satisfyingAssignmentMade v(0) v(5) v(3) **
              watchAfterSatisfyingAssignment v(0) v(5) v(3) v(2)) *\/*

            (* Watch variable invariant case 3: both watch variables point to
               unassigned variables *)
            watchesUnassigned v(0) v(5) v(3)))))))))))).
